...
function htmlBody(p: SubmitPayload): string {
  ...
  <div style="font-family: -apple-system, BlinkMacSystemFont, sans-serif; max-width: 560px; margin: 0 auto; padding: 24px; background: #F2F0EB; color: #3D4450;">
    <div style="background: #0F1E3A; color: #fff; padding: 24px; border-radius: 8px 8px 0 0;">
      <div style="color: #4A9B93; font-size: 11px; letter-spacing: 3px; text-transform: uppercase;">Operaria Flow</div>
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { error } = await resend.emails.send({
      from,
      to,
      subject: `Diagnóstico completado: ${payload.nombreFormateado}`,
      html: htmlBody(payload),
      attachments: [
        {
          filename: `diagnostico-${payload.cliente}.pdf`,
          content: pdfBuffer,
        },
      ],
    });
